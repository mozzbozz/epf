...
class Session(object):
    ...
    def start(self):
        """
        Starts the prompt once the session is prEPFred
        """
        # self.logger.log_info("Starting EPF Session")
    ...
    def run_all(self):
        ...
        """
        # NOTE: MAIN LOOP ENTRY
        while not self.is_paused:
            for species, population in self.populations.items():
                for i in range(0, 10000):
                    test_case = TestCase(id=0, session=self, individual=population.new_child(), path=[])
                    try:
                        self.is_paused = test_case.run()
                    except Exception:
                        pass
            # fuzzable = next(self.fuzzables)
            # print(fuzzable)
    ...
    def run_next(self, force: bool = False):
        ...
        """
        run = True
        # WE NEED A MECHANISM TO FUZZ HERE
        if self.test_case is not None:
