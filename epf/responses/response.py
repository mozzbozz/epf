...
from epf.exception import EPFRuntimeError
#from epf.mutants import blocks
...
class Response(object, metaclass=ABCMeta):
    ...
    def parse(self, data: bytes):
        ...
        Raises EPFRuntimeException if not all required_variables are set
        """
    ...
    def _check_vars(self, vars_set: Mapping[str, bytes]):
        ...
        Raises EPFRuntimeError if a required_var was not set by _extract_variables()
        """
        for var in self.required_vars:
            if vars_set is None or var not in vars_set or vars_set[var] is None:
                self._empty_vars()
                raise EPFRuntimeError(f'The variable {var} was not set')
