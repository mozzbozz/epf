...
from fuzzowski.connections import base_socket_connection, ip_constants
        ...
        try:
            if self.server:
                if self._udp_client_port is None:
                    raise exception.FuzzowskiError("recv() must be called before send with udp fuzzing servers.")
