...
from fuzzowski.connections import base_socket_connection
        ...
        if self.server:
            ...
            try:
                ...
                if e.errno == errno.EADDRINUSE:
                    raise exception.FuzzowskiOutOfAvailableSockets()
                else:
                    raise
            ...
            try:
                ...
                if e.errno in [errno.EAGAIN]:
                    raise exception.FuzzowskiTargetConnectionFailedError(str(e))
                else:
                    raise
    ...
    def recv(self, max_bytes):
        ...
        try:
            ...
            if e.errno == errno.ECONNABORTED:
                raise_(
                    exception.FuzzowskiTargetConnectionAborted(socket_errno=e.errno, socket_errmsg=e.strerror),
                    None,
                    sys.exc_info()[2],
                )
            elif (e.errno == errno.ECONNRESET) or (e.errno == errno.ENETRESET) or (e.errno == errno.ETIMEDOUT):
                raise_(exception.FuzzowskiTargetConnectionReset(), None, sys.exc_info()[2])
